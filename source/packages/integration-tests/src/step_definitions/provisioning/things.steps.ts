...
Given('thing {string} exists', async function (deviceId: string) {
    try {
        ...
        const describeThingResponse = await iot.describeThing(describeThingRequest);
...
Then('the thing {string} is provisioned', async function (thingName: string) {
    ...
    const describeThingResponse = await iot.describeThing({ thingName });
    ...
    await iot.describeCertificate({ certificateId });
});
...
Then(
    ...
    async function (thingName: string, thingGroupName: string) {
        ...
        const r = await iot.listThingGroupsForThing({ thingName });
        const exists =
